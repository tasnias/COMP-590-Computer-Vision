...
def cross_correlation_2d(im, kernel, path='same', padding='zero'):
    ...
    if path == 'full':
        ...
        # Crop by 2k on all sides to get only valid pixels
        return output[2*k : output.shape[0] - 2*k, 2*k : output.shape[1] - 2 * k]
    else: 
        # Crop by k on all sides to get same size image
...
#blurred = convolve_2d(im, kernel, "valid", "zero")
#cv2.imshow('blurred.jpg', blurred)
#cv2.waitKey(0)
#cv2.imwrite('resized2.jpg', image_shrinking(im, (im.shape[1]/2, im.shape[0]/2)))
#cv2.waitKey(0)
...
#im = np.array([[0,1,2,2,7],[0,1,1,6,10],[3,3,9,14,17],[4,7,13,19,18],[8,15,20,22,25]])
#print(im)
#sobelx, sobely = sobel_kernel()
#dy = convolve_2d(im, sobely, "valid", "zero")
#print(dy)


def compute_M():
    image = np.array([[1,1,6],[3,9,14],[7,13,19]])
    I_x = sobel(image, 0)
    I_y = sobel(image, 1)
    A = I_x ** 2
    B = I_y ** 2
